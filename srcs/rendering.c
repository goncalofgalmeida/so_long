...
void	load_images(t_game *game)
{
	...
	sprites->grass = mlx_xpm_file_to_image(game->mlx.init, GRASS_PATH, &width, &height);
	sprites->wall = mlx_xpm_file_to_image(game->mlx.init, TREE_PATH, &width, &height);
	sprites->collectible = mlx_xpm_file_to_image(game->mlx.init, COL1_PATH, &width, &height);
	sprites->player = mlx_xpm_file_to_image(game->mlx.init, PLAYER_PATH, &width, &height);
	if (!sprites->grass || !sprites->wall || !sprites->collectible || !sprites->player)
	{
